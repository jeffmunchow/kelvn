 ...
 * /api/newsletter
 *
 * GET ?action=unsub&uid=<user_id>  → cancela recebimento (link do email)
 * GET/POST (sem action)            → envia newsletter semanal (cron)
 */
module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  if (req.method === 'OPTIONS') return res.status(200).end();

  if (req.query.action === 'unsub') return acUnsub(req, res);
  return acEnviar(req, res);
};
...
function buildEmail({ nome, email, userId, prospects, eventosSemana, semContrato,
  ...
  const unsubUrl = `https://app.kelvn.com.br/api/newsletter?action=unsub&uid=${encodeURIComponent(userId)}`;
